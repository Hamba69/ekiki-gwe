...
export function buildCardOrder(seed, cardIndexes = Array.from({ length: CARD_COUNT }, (_, index) => index)) {
...
export function applyGameAction(room, action, playerId) {
  ...
  if (action === "next") {
    ...
    const cardCount = Number.isInteger(next.cardCount) && next.cardCount > 0
      ? next.cardCount
      : CARD_COUNT;
